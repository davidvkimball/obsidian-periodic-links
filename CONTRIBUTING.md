...
Build artifacts (`main.js`, `manifest.json`) are produced at the repo root.

To test against a real vault, point the build output at `<vault>/.obsidian/plugins/<plugin-id>/` (symlink the files, or use a tool like [hot-reload](https://github.com/pjeby/hot-reload)).
...
Maintainers cut releases by bumping the version in `manifest.json`, `package.json`, and `versions.json`, committing, then tagging and pushing the tag. The release workflow builds and attaches `main.js` and `manifest.json` with build provenance attestation.
